...
function APContent() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:3333/getSiteAP/' + paramPath).then(res => {
      const data = res.data
      let inDoorCount = 0
      let outDoorCount = 0
      data.map((item) => {
        ...
        <Container fluid>
          ...
          <Navbar.Collapse id="navbar-dark-example">
            <Nav className="me-auto">
              ...
              >
              <NavDropdown.Item onClick={(e) => handleParamUpdate("AP", "Datasheet")}>Add New Datasheet</NavDropdown.Item>
              ...
              <NavDropdown title="Access Point Model" id="basic-nav-dropdown">
                <NavDropdown.Item onClick={(e) => handleParamUpdate("AP", "Model")}>Add New Model</NavDropdown.Item>
      ...
      <div className="vh-100 justify-content-center align-items-center">
        <div className="bg-white p-3">
          ...
          >
            {paramPath !== "APList" && (
              <>
                <h2>Access Point List ({paramPath}) : {num_ap_site} Units</h2>
              </>
            )}
            {paramPath === "APList" && (
              <>
                <h2>Access Point List : {num_ap} Units</h2>
              </>
            )}
          </div>
            ...
            <tbody>
              {aplist.map((aplist, index) => (
                <tr key={index}>
                  ...
                  <td>{aplist.MACaddress}</td>   
                  <td>
                    <Link
                      to={`/device_log/${aplist.ID}`}
